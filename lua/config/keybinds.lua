...
vim.keymap.set("n", '<leader>pp', "<cmd>lua vim.pack.update(nil, { offline = true })<CR>", { desc = 'show installed plugins'})
...
vim.keymap.set("n", "<leader>lS", "<cmd>lua Snacks.picker.lsp_workspace_symbols()<CR>", { desc = "LSP Workspace Symbols" } )
...
vim.keymap.set('n', "<leader>dX", "<cmd>Trouble diagnostics toggle filter.buf=0<cr>", { desc = "(Trouble) Buffer Diagnostics", } )
vim.keymap.set('n', "<leader>ds", "<cmd>Trouble symbols toggle focus=false<cr>", { desc = "(Trouble) Symbols", } )
vim.keymap.set('n', "<leader>dl", "<cmd>Trouble lsp toggle focus=false win.position=right<cr>", { desc = "(Trouble) LSP Definitions / references", } )
